...
$(function () {
    ...
    function scrennShot(bool, streamID, video) {
      ...
      if(IsPC()){
          ...
          var config = {
              ...
              height:480,
              frameRate:15,
              bitRate:$('#screenBitRate').val() * 1 || 1500
          };
    ...
    $('#mixAudio').click(function () {
      ...
      let mixAudioList = [];
      $('.remoteVideo video').map((ind, video) => {
        video.srcObject.getAudioTracks().length > 0 && mixAudioList.push(video);
      });

      if (mixAudioList.length > 0 && ( (previewVideo.srcObject && previewVideo.srcObject.getAudioTracks().length > 0) || (!!$('.screenShareVideos video:last-child')[0] && $('.screenShareVideos video:last-child')[0].srcObject.getAudioTracks().length > 0)) ) {
        mixStreamID = $('.screenShareVideos video:last-child')[0] ? $('.screenShareVideos video:last-child')[0].id : _config.idName;
        zg.startMixingAudio(mixStreamID, mixAudioList)
      } else{
        alert('当前条件不满足混音，请检查被混流是否有音轨');
      }

    })

    $('#stopMixAudio').click(function () {
      mixStreamID && zg.stopMixingAudio(mixStreamID)
    })
